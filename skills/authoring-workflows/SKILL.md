...
description: Binds whenever a plan owes its execution workflow — any T2/T3 planning
  brief where the madhyastha is about to write the Workflow section. That section is
  emitted as a runnable Workflow-tool script, not prose for the chair to translate;
  the chair runs it verbatim or returns it for revision. Load before writing any
  `export const meta` line. Carries the emission contract, script-shape recipe, and
  pre-emission checklist. Not a Workflow-tool tutorial; whether a script is owed at
  all is CLAUDE.md §5's decision, not this skill's.
---
...
Put rationale in script comments; `/home/xzat/.claude/workflows/pr-review.js`
lines 34–59 are the model. Never duplicate the graph as a parallel prose station
table: that creates a second translation surface, and it will drift from the script.
...
`async function f(){ ... }`, and running `node --check`. Bare `node --check` on the
raw file **fails by design** because top-level `return` is valid to the Workflow tool,
not to Node's file grammar. Fix the check invocation, never the script. Do not
mutilate a correct workflow to satisfy the wrong checker.
...
if (!a.today) throw new Error(
  'args.today required (YYYY-MM-DD). Scripts cannot call Date.now() — ' +
...
Pattern map: meta literal → `pr-review.js` lines 1–12; args throw → lines 18–20;
lane-rationale comments → lines 34–59; pipeline-not-parallel reasoning → lines
168–170; light dispatch → lines 174–210; gap accounting → lines 241–266; reviewer
routing → `review-station.js` lines 12–34.
...
`pr-review.js` lines 213–238. Add a dedicated structurer only when no Claude station
already follows.
...
standing session corrupts it (`pr-review.js` lines 180–182). Say why in a comment.
...
`pr-review.js` lines 241–266 are the reference implementation. A failed station is
never a pass.
...
- [ ] Replay-sensitive time and randomness enter through validated args; no `Date.now()` or `Math.random()`.
...
extraction; schema killing codex-wrapper; resume replay divergence under ambient
time; and the raw-file `node --check` failure mode. These agree with live-script
comments and CLAUDE.md §5.
